...
<#
.SYNOPSIS
    Deploys signalR using azuredeploy.json
.DESCRIPTION
    Deploys a signalR to the resource group.
...
.\Deploy-SignalR.ps1  `
    ...
    -ResourceGroupName $resourceGroupName -ResourceName "sigr-wus2-testme" `
...
Param(
    ...
    [Parameter(Mandatory=$false)] [string] $ResourceName = "sigr-wus2-test-99",
    [Parameter(Mandatory=$false)] [array] $AllowedOrigins = @("https://foo1.com","https://bar2.com")
)
...
try {
    ...
    $resourceName = (Get-AzResourceGroupDeployment `
        -ResourceGroupName $ResourceGroupName `
        -Name $deploymentName).Outputs.signalrResourceName.value
