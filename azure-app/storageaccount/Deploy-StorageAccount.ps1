...
<#
.SYNOPSIS
    Deploys a storage account using azuredeploy.json
.DESCRIPTION
...
.EXAMPLE
  ...
  .\Deploy-StorageAccount.ps1 -SubscriptionID 76d5aee2-6a43-4ec9-8248-77f888fb917c `
...
try {
    ...
    $resourceID = (Get-AzResourceGroupDeployment `
        -ResourceGroupName $ResourceGroupName `
        -Name $deploymentName).Outputs.storageAccountResourceId.value
